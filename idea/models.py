...
class Idea_image_storage1(models.Model):
    idea = models.ForeignKey(Idea1, on_delete=models.CASCADE)
    image = models.ImageField(upload_to="", null = True, blank = True)

    def __str__(self):
        return str(self.idea_id) + '번 아이디어 ' + str(self.id) +'번 사진'

class Idea_Comments1(models.Model):  # (어떤 유저의 어떤 아이디어에) 다른 유저가 다는 댓글들 저장
    idea = models.ForeignKey(Idea1, on_delete=models.CASCADE, null = True, blank = True)    # 어떤 유저의 어떤 아이디어에
...
class Idea_AddComments1(models.Model):   # (어떤 유저의 어떤 아이디어에 달린 댓글들에) 다른 유저가 작성하는 대댓글들 저장
    idea_comments = models.ForeignKey(Idea_Comments1, on_delete=models.CASCADE, null = True, blank = True)  # 어떤 유저의 어떤 아이디어에 달린 댓글들
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null = True, blank = True)    # 다른 유저가
    text = models.CharField(max_length = 200, blank = True, null = True)    # 작성하는 대댓글
    create_data = models.DateTimeField(default = timezone.now) # 생성날짜
